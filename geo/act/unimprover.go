...
func (w WrappedTrack) SpeedReported() float64 {
	return w.Properties.MustFloat64("Speed", -1)
}
func (w WrappedTrack) HeadingReported() float64 {
...
func (c *Cat) push(ct WrappedTrack) {
	...
	c.WindowSpeedReportedSum += ct.SpeedReported() * ct.TimeOffset.Seconds()
...
func (p *Improver) isNapLapTransition(ct WrappedTrack) bool {
	...
	if IsActivityActive(p.Cat.ActivityState) {
		...
		if diff := ct.SpeedCalculated / ct.SpeedReported(); diff > 1.5 {
			tx += diff - 1.5
		}
...
func (p *Improver) improve(ct WrappedTrack) error {
	...
	if !p.Cat.IsWindowEmpty() {
		...
		accelerationReported = (ctReportedSpeed - p.Cat.Last.SpeedReported()) / timeOffset.Seconds()
	...
	ct.HeadingDeltaReported = math.Abs(ctHeading - p.Cat.Last.HeadingReported())
